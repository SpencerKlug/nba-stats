    ...
    if include_games or include_box_scores:
        ...
        if contest_ids:
            if include_box_scores:
                box_df, schedule_from_box = box_score.load_player_box_scores_and_schedule(
                    contest_ids, season, limit=limit
                )
                if not box_df.empty:
                    result["player_box_scores"] = box_df
                if include_games and not schedule_from_box.empty:
                    result["schedule"] = schedule_from_box
            elif include_games and not games_df.empty:
                result["schedule"] = games_df
